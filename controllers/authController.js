...
exports.protect = asyncHandler(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError("You are not login, please login to access this route")
    );
  }
  ...
  if (!currentUser) {
    return next(
      new AppError("The user that belong to this token does no longe exist"),
      401
    );
  }
  ...
  if (currentUser.passwordChangedAt) {
    ...
    if (passChangedTimestamp > decoded.iat) {
      return next(
        new AppError("User recently changed his password, Please login again"),
        401
      );
    }
  }
